...
**Last updated:** Milestone 87A Finalization — Core Governance Authorization Decision-Envelope Boundary (COMPLETE; committed at pending; tagged pending; pushed pending; Milestone 87 remains open; Milestone 87B not started; Milestone 88 not started)
**Aether version:** 0.2.0  
**Current completed local milestone:** Milestone 87A Finalization — Core Governance Authorization Decision-Envelope Boundary (complete locally; production source and runtime behavior unchanged; finalized: pending)
**Current active milestone/module:** None; Milestone 87A validation is complete and Milestone 87B has not started
**Current status:** Milestone 87A boundary record and tests COMPLETE locally: accepted Milestone 87 direction is Core Governance Authorization Decision-Envelope Foundation; decision record `docs/architecture/MILESTONE_87_CORE_GOVERNANCE_AUTHORIZATION_BOUNDARY.md` created with the exact 24-section contract; 50/50 new boundary tests passed in `tests/test_milestone_87_core_governance_authorization_boundary.py`; direct production importer inventory proves `aether/core/loop.py` is the sole production importer of `enforce_policy_gate`; Core Governance owns the authoritative decision while Thinking proposes and Verification/Identity provide direct evidence; `allowed` is explicitly not execution permission; approval request/pending/granted/execution states remain separate; selected future physical home `aether/core/governance.py`; selected 87B migration is one authoritative Governance implementation with `aether/action/policy_gate.py` converted to a thin compatibility facade and the core loop atomically redirected; tentative 87B matrix is `aether/core/governance.py`, `aether/core/loop.py`, `aether/action/policy_gate.py`, the 87A boundary test, and post-validation PROGRESS.md; existing Governance/policy/core/protected regression 592 passed; architecture and Observation suite 240 passed; PROGRESS consistency 55 passed; combined lightweight suite 136 passed; full suite 2187/2187 passed, 0 failures, 0 errors, 9 existing warnings; OpenAPI 304/108; api_server 8 @app routes / 23 include_router / zero direct /action/*; Constitution SHA unchanged at `0055748f683bf753b3471a0317b68677752c312d4030b12fbc71684fd3af3ee1`; Architecture v0.3.0 unchanged; `git diff -- aether/` empty; no existing test or architecture record modified; tracked private/runtime empty; docs/history clean; drift 0 at canonical fingerprint `600fd549588be7f536f704bc999be1987dcdf550225f2dc11dbf2fbf63ec2bcd`; no runtime extraction, behavior change, execution, persistence, API/router/model/schema, commit, tag, or push; Milestone 87 remains open; Milestone 87B has not started; Milestone 88 has not started; Finalization: pending (implementation commit pending; tag pending; push pending; ledger pending)
...
**Test baseline:** 2187/2187 passed, 0 failures, 0 errors (2137 accepted baseline + 50 new Milestone 87A boundary tests; 9 existing warnings, no new warnings)
**OpenAPI baseline:** 304 paths / 108 schemas
**Latest local tag:** `milestone-86A-architecture-evolution` at `4e82f6a` (Milestone 86A implementation commit); `milestone-85A-observe-verify-lifecycle-boundary-record` at `b5bad54`
**Latest pushed GitHub/origin status:** `origin/main` == HEAD == `b3e0070` (Milestone 86A finalization-ledger commit); remote tag `milestone-86A-architecture-evolution` remains at `4e82f6a`; Milestones 83-86 are closed; Milestone 87A is not committed, tagged, pushed, or finalized; Milestone 87 remains open; Milestone 87B has not started; Milestone 88 has not started
...
- 82AL Build Part 1 — Repair Family State-Boundary Tests
  ...
  - 87A — Core Governance Authorization Decision-Envelope Boundary Record and Contract Tests (COMPLETE locally; finalized: pending; implementation commit pending; tag pending; push pending; Milestone 87 not closed; Milestone 87B not started)
    - accepted Milestone 87 direction: Core Governance Authorization Decision-Envelope Foundation
    - task classification: design-record and tests-only boundary Build
    - decision record created: `docs/architecture/MILESTONE_87_CORE_GOVERNANCE_AUTHORIZATION_BOUNDARY.md`; exact H1; exact 24 top-level sections; authoritative-until-revised clause; no runtime implementation claim
    - boundary suite created: `tests/test_milestone_87_core_governance_authorization_boundary.py`; 50/50 passed; static and pure deterministic calls only; no TestClient, endpoint invocation, persistence, tool/action execution, configuration mutation, or network access
    - production chain locked: `/chat` -> Interface -> `AetherRuntime.process_chat` -> `run_core_chat_loop` -> Perception -> Identity evidence -> Time facts -> Working Memory -> Verification risk evidence -> Thinking proposal -> current policy gate -> approval builder -> optional pending approval record -> response/trace/Timeline
    - direct importer inventory: `aether/core/loop.py` is the only production importer of `enforce_policy_gate`; `tests/test_policy_gate.py` is the existing direct test importer
    - ownership locked: Thinking proposes; Verification and Identity supply direct evidence; Core Coordination invokes; Core Governance authorizes; Action and approval mechanics remain downstream
    - envelope locked: `allowed`, `decision`, `reason`, `required_user_confirmation`, `tool_execution_allowed`, `action_execution_allowed`, `requested_action`, `policy_snapshot`, `warnings`; current signature/defaults and invalid/block/require-approval/deny/legacy synthetic-allow compatibility behavior preserved
    - `allowed` explicitly is not tool/action execution permission, completed confirmation, or Human Authority approval; proposal request, Governance approval requirement, approval request, pending approval, granted approval, and execution authorization remain separate
    - current `/chat` no-execution invariants locked: tool/action execution false; tool executed false; no real apply, rollback, evidence collection, external side effect, Observation-driven action, or automatic execution from approval
    - selected future physical home: `aether/core/governance.py` (not created in 87A); dependency direction: Thinking/Verification/Identity values -> Core Coordination caller -> Core Governance decision -> Action/approval/response/trace consumers; Governance imports neither loop nor Interface
    - selected 87B migration: move one authoritative implementation to Core Governance, redirect the core loop atomically, and convert `aether/action/policy_gate.py` to a thin compatibility facade; no parallel authoritative gates
    - exact tentative 87B changed-file matrix: create `aether/core/governance.py`; modify `aether/core/loop.py`; modify `aether/action/policy_gate.py`; update `tests/test_milestone_87_core_governance_authorization_boundary.py`; update `PROGRESS.md` only after all 87B gates; external summary only outside Git
    - validation: new boundary 50 passed; existing policy/gate/core/approval/risk/identity/trace/protected suite 592 passed; architecture and Observation suite 240 passed; PROGRESS consistency 55 passed; combined lightweight suite 136 passed; full suite 2187/2187 passed, 0 failures, 0 errors, 9 existing warnings
    - structural/integrity: OpenAPI 304/108; api_server 8/23/0; Architecture v0.3.0; Constitution SHA `0055748f683bf753b3471a0317b68677752c312d4030b12fbc71684fd3af3ee1` unchanged; production source unchanged; all existing tests and records unchanged; tracked private/runtime empty; docs/history clean; canonical drift 0 at `600fd549588be7f536f704bc999be1987dcdf550225f2dc11dbf2fbf63ec2bcd`
    - production source changed: no; runtime behavior changed: no; runtime extraction performed: no
    - finalized: no; committed: no; tagged: no; pushed: no
    - Milestone 87: OPEN; Milestone 87B started: no; Milestone 88 started: no
    - Next: human/project-manager review of Milestone 87A; after explicit acceptance, Milestone 87B — Core Governance Authorization Decision-Envelope Extraction
